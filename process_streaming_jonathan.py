...
input_file_name = "MOCK_DATA.csv"
output_file_name = "MOCK_DATA_2.csv"
# read from a file to get some fake data
input_file = open("MOCK_DATA.csv", "r")
output_file = open("MOCK_DATA_2.csv", "w", newline='')
...
for row in reader:
    ...
    id,first_name,last_name,email,gender,ip_address = row
    
    # use an fstring to create a message from our data
    # notice the f before the opening quote for our string?
    fstring_message = f"[{id}, {first_name}, {last_name}, {email},{gender}, {ip_address}]"
...
header = next(reader)
header_list = ["id","first_name","last_name","email","gender","ip_address"]
csv.writer.writerow(header_list)

csv.writer.writerow([id,first_name, last_name, email, gender, ip_address])
